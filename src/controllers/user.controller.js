...
const getUserChannel = asyncHandler( async (req, res) => {
    ...
    // Aggregate(channel) Array return karega
    return res
    .status(200)
    .json(
...
const getWatchHistory  = asyncHandler(async(req, res) => {
    const user =  await User.aggregate([
        {
            $match: {
                _id: new mongoose.Types.ObjectId(req.user._id) // req.user._id -> ye hume string detat hai -> ObjectId('8764304rq4uiegrqhjirrhg) joki actuatl id nahi hota hai , mongoose ki help se hume iske anader ki id milti hai  
            }
        },
        {
            $lookup: {
